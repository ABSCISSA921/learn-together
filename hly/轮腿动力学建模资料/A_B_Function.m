    ...
    % 定义参数符号
    syms Im Ip Iw L Lm M R g l mp mw real

    % 将参数打包成列表
    params = [Im, Ip, Iw, L, Lm, M, R, g, l, mp, mw];
    ...
    A=subs(jacobian([d_theta,f1,d_x,f2,d_phi,f3],[theta0,d_theta,x0,d_x,phi0,d_phi]),[theta0,d_theta,d_x,phi0,d_phi,T,Tp],[0.0,0,0,0,0,0,0]);
    % A=subs(A,[R,L,LM,l,mw,mp,M,Iw,Ip,IM,g],[R1,L1,LM1,l1,mw1,mp1,M1,Iw1,Ip1,IM1,9.81]);
    % A=double(A);
    B=subs(jacobian([d_theta,f1,d_x,f2,d_phi,f3],[T,Tp]),[theta0,d_theta,d_x,phi0,d_phi,T,Tp],[0.0,0,0,0,0,0,0]);
    % B=subs(B,[R,L,LM,l,mw,mp,M,Iw,Ip,IM,g],[R1,L1,LM1,l1,mw1,mp1,M1,Iw1,Ip1,IM1,9.81]);
    % B=double(B);


    %% 把之前的 A、B 表达式用 subs 替换成参数符号版本
    A_sym = subs(A,[IM,Ip,Iw,L,LM,M,R,g,l,mp,mw],params);
    
    B_sym = subs(B,[IM,Ip,Iw,L,LM,M,R,g,l,mp,mw],params);

    %% 利用 matlabFunction 生成函数文件
    matlabFunction(A_sym, 'Vars', params, ...
        'File', 'gen_A.m', 'Outputs', {'A'});
    
    matlabFunction(B_sym, 'Vars', params, ...
        'File', 'gen_B.m', 'Outputs', {'B'});